...
LOG_FILE=${LOG_DIR}serializedmaterialExtract.log_"$dt"_1
...
if [ -e ${LOG_DIR}serializedmaterialExtract.lck ]
then
...
touch ${LOG_DIR}serializedmaterialExtract.lck
chmod 777 ${LOG_DIR}serializedmaterialExtract.lck


#Starting the cassandra extraction spark job execution
/app/UDMF/spark/bin/spark-submit \
--class com.tmobile.dlmExtract.serializedmaterialExtract \
--name serializedmaterialExtract \
--master yarn \
--deploy-mode cluster \
...
if [ $? -eq 0 ]
then
    echo "$location serializedmaterial extraction is completed successfully"
    #echo "Coping the data to monday date file"
    rm -f ${LOG_DIR}serializedmaterialExtract.lck
    echo "Lock Released" >> ${LOG_FILE}
else
        echo "$location serializedmaterial extraction job has failed!!" >> ${LOG_FILE}
        cd $LOG_DIR
        ...
        do
            ...
            ( printf "Dear All,"
                        printf '%s\n'
                        printf '%s\n'
                        printf '%s\n' "$location serializedmaterial extraction job Failed."
                        printf '%s\n' "Please find the attached error stack log for analysis."
                        printf '%s\n'
                        printf '%s\n' "Thanks,"
                        printf '%s\n' "DLM-Extraction Team" ) | mailx -s "$location serializedmaterial extraction job failed" -a ${LOG_DIR} $sparkfile1 mounica.arepalli1@t-mobile.com
        done
        rm -f ${LOG_DIR}serializedmaterialExtract.lck
        echo "Lock Released" >> ${LOG_FILE}
fi