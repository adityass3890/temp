...
LOG_FILE=${LOG_DIR}serializeddevicehistoryV2Extract.log_"$dt"_1
...
if [ -e ${LOG_DIR}serializeddevicehistoryV2Extract.lck ]
then
...
touch ${LOG_DIR}serializeddevicehistoryV2Extract.lck
chmod 777 ${LOG_DIR}serializeddevicehistoryV2Extract.lck


#Starting the cassandra extraction spark job execution
${SPARK_HOME}bin/spark-submit \
--class com.tmobile.dlmExtract.serializeddevicehistoryV2Extract \
--name serializeddevicehistoryV2Extract \
--master yarn \
--deploy-mode cluster \
...
--executor-memory 6g \
--executor-cores 8 \
${JAR_DIR}tetra-elevate-conversion_2.12-1.0.jar ${CONF_DIR} extract_config.json TETRA $1

if [ $? -eq 0 ]
then
    echo "$location serializeddevicehistoryV2 extraction is completed successfully"
    #echo "Coping the data to monday date file"
    rm -f ${LOG_DIR}serializeddevicehistoryV2Extract.lck
    echo "Lock Released" >> ${LOG_FILE}
else
        echo "$location serializeddevicehistoryV2 extraction job has failed!!" >> ${LOG_FILE}
        cd $LOG_DIR
        ...
        do
            ...
            ( printf "Dear All,"
                        printf '%s\n'
                        printf '%s\n'
                        printf '%s\n' "$location serializeddevicehistoryV2 extraction job Failed."
                        printf '%s\n' "Please find the attached error stack log for analysis."
                        printf '%s\n'
                        printf '%s\n' "Thanks,"
                        printf '%s\n' "DLM-Extraction Team" ) | mailx -s "$location dualimeiinfobyprimaryimei extraction job failed" -a ${LOG_DIR} $sparkfile1 mounica.arepalli1@t-mobile.com
        done
        rm -f ${LOG_DIR}serializeddevicehistoryV2Extract.lck
        echo "Lock Released" >> ${LOG_FILE}
fi
