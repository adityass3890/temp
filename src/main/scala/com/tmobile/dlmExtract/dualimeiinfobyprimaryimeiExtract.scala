...
object dualimeiinfobyprimaryimeiExtract {

  @transient lazy val logger = org.apache.log4j.LogManager.getLogger(dualimeiinfobyprimaryimeiExtract.getClass)
  ...
  def main(args: Array[String]): Unit = {
    ...
    CommonMethods.log4jLogGenerate(logger, extractConfig.getString("warehouse.logdir"), "dualimeiinfobyprimaryimeiExtract")
    ...
    val conf = new SparkConf(true).setAppName("dualimeiinfobyprimaryimeiExtract")
    ...
    val targetTable = "dualimeiinfo_by_primary_imei"
    ...
    extractionDf = extractionDf.selectExpr(extractionDf.columns.map(c => s"cast($c as string) as $c"): _*)
