...
object stocktransferorderIngest {
  ...
  def main(args: Array[String]): Unit = {
    ...
    var extractionDf = CommonMethods.readFromCSVFile(outboundPath + "/*part*", "|", spark, SchemaDefinition.stocktransferorderSchema)
    
    // Remove duplicates based on primary key (stonumber, destinationpoint)
    extractionDf = extractionDf.dropDuplicates("stonumber", "destinationpoint")
