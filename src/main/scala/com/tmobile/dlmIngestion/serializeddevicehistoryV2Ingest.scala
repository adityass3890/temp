...
object serializeddevicehistoryV2Ingest {

  @transient lazy val logger = org.apache.log4j.LogManager.getLogger(serializeddevicehistoryV2Ingest.getClass)
  ...
  def main(args: Array[String]): Unit = {
    ...
    CommonMethods.log4jLogGenerate(logger, extractConfig.getString("warehouse.logdir"), "serializeddevicehistoryV2Ingest")
    val start_time = System.nanoTime
    val timeFormat = new SimpleDateFormat("YYYY-MM-dd HH:mm:ss")
    var cal = Calendar.getInstance()
    val processed_starttime = timeFormat.format(cal.getTime).toString()

    logger.info("<----------- transaction extraction has started ---------------->")
    logger.info("transaction - extraction started on " + processed_starttime)

    val conf = new SparkConf(true).setAppName("serializeddevicehistoryV2Ingest")
    ...
    val targetTable = "serialised_devices_history"
    //val postDateFilter=args(3)

    val outboundPath = extractConfig.getString("warehouse.outbound_path") + "/" + sourceTable

    extractionDf = CommonMethods.readFromCSVFile(outboundPath + "/*part*", "|", spark, SchemaDefinition.serializeddevicehistorySchema)
